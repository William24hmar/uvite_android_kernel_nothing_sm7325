...
GCC64_DIR="$HOME/toolchains/aarch64-none-linux-gnu"
...
GCC32_DIR="$HOME/toolchains/arm-linux-androideabi-4.9"
...
setup_toolchains() {
	...
	if [ -f "$SDCLANG_DIR/bin/clang" ]; then
		...
		echo -e "${YELLOW}Downloading Snapdragon Clang 19 (~978 MB)…${NC}"
		wget -q --show-progress -O /tmp/sdclang19.tgz "$SDCLANG_URL" \
			|| { echo -e "${RED}✘ Snapdragon Clang download failed!${NC}"; exit 1; }
		echo -e "${YELLOW}Extracting Snapdragon Clang…${NC}"
		# sdclang19.tgz has NO top-level subdirectory — bin/ lib/ etc. are at
		# the root of the archive. Extract directly into SDCLANG_DIR (no --strip-components).
		rm -rf "$SDCLANG_DIR"
		mkdir -p "$SDCLANG_DIR"
		tar -xf /tmp/sdclang19.tgz -C "$SDCLANG_DIR" \
			|| { echo -e "${RED}✘ tar failed extracting sdclang19.tgz${NC}"; exit 1; }
		rm -f /tmp/sdclang19.tgz
		[ -f "$SDCLANG_DIR/bin/clang" ] \
			|| { echo -e "${RED}✘ Extraction failed — clang binary missing in $SDCLANG_DIR/bin/${NC}"; exit 1; }
		chmod +x "$SDCLANG_DIR/bin/"*
	...
	if [ -f "$GCC64_DIR/bin/aarch64-none-linux-gnu-gcc" ]; then
		...
		local tmp_gcc64
		tmp_gcc64=$(mktemp -d /tmp/gcc64_extract_XXXXXX)
		tar -xf /tmp/gcc64.tar.xz -C "$tmp_gcc64" \
			|| { echo -e "${RED}✘ tar failed extracting gcc64${NC}"; rm -rf "$tmp_gcc64"; exit 1; }
		rm -f /tmp/gcc64.tar.xz
		rm -rf "$GCC64_DIR"
		local extracted_gcc64
		extracted_gcc64=$(ls -d "$tmp_gcc64"/*)
		mv "$extracted_gcc64" "$GCC64_DIR"
		rm -rf "$tmp_gcc64"
		[ -f "$GCC64_DIR/bin/aarch64-none-linux-gnu-gcc" ] \
			|| { echo -e "${RED}✘ GCC64 extraction failed — binary missing in $GCC64_DIR/bin/${NC}"; exit 1; }
	...
	# ── Also export CC/LD as environment vars — these propagate into every
	# ── sub-make invocation and override the Makefile's bare-name defaults
	# ── even when LLVM= is not set. The command-line CC= in MAKE_ARGS takes
	# ── the highest precedence, but the env export is a belt-and-braces guard.
	export CC="$SDCLANG_DIR/bin/clang"
	export CXX="$SDCLANG_DIR/bin/clang++"
	export LD="$SDCLANG_DIR/bin/ld.lld"
	export AR="$SDCLANG_DIR/bin/llvm-ar"
	export NM="$SDCLANG_DIR/bin/llvm-nm"
	export OBJCOPY="$SDCLANG_DIR/bin/llvm-objcopy"
	export OBJDUMP="$SDCLANG_DIR/bin/llvm-objdump"
	export STRIP="$SDCLANG_DIR/bin/llvm-strip"
...
# ── Immediately verify the sdclang binary exists before going any further ──
# This catches path resolution issues early with a clear error message.
echo -e "${CYAN}[VERIFY] SDCLANG_DIR = $SDCLANG_DIR${NC}"
echo -e "${CYAN}[VERIFY] GCC64_DIR   = $GCC64_DIR${NC}"
echo -e "${CYAN}[VERIFY] GCC32_DIR   = $GCC32_DIR${NC}"
if [ ! -f "$SDCLANG_DIR/bin/clang" ]; then
	echo -e "${RED}[FATAL] Snapdragon Clang binary not found at: $SDCLANG_DIR/bin/clang${NC}"
	echo -e "${RED}        setup_toolchains() failed silently — check download/extract steps above${NC}"
	exit 1
fi
echo -e "${GREEN}[VERIFY] Snapdragon Clang OK: $($SDCLANG_DIR/bin/clang --version 2>&1 | head -1)${NC}"
...
# Use absolute paths into the Snapdragon Clang bin dir so make NEVER falls back
# to any system clang/lld/llvm-* regardless of $PATH ordering.
TARGET_CC="$SDCLANG_DIR/bin/clang"
TARGET_CXX="$SDCLANG_DIR/bin/clang++"
TARGET_HOSTLD="$SDCLANG_DIR/bin/ld.lld"
TARGET_LD="$SDCLANG_DIR/bin/ld.lld"
TARGET_AR="$SDCLANG_DIR/bin/llvm-ar"
TARGET_NM="$SDCLANG_DIR/bin/llvm-nm"
TARGET_OBJCOPY="$SDCLANG_DIR/bin/llvm-objcopy"
TARGET_OBJDUMP="$SDCLANG_DIR/bin/llvm-objdump"
TARGET_STRIP="$SDCLANG_DIR/bin/llvm-strip"
TARGET_CLANG_TRIPLE="aarch64-none-linux-gnu-"
# Bare prefixes for CROSS_COMPILE= — the Makefile's notdir/target= logic
# works correctly with these; PATH (set by setup_toolchains) finds the binaries
TARGET_CROSS_COMPILE_PREFIX="aarch64-none-linux-gnu-"
TARGET_CROSS_COMPILE_COMPAT_PREFIX="arm-linux-androideabi-"
# Absolute-prefixed forms (kept for reference / any direct shell calls)
TARGET_CROSS_COMPILE="$GCC64_DIR/bin/aarch64-none-linux-gnu-"
...
TARGET_OUT="$HOME/NOTHING_PHONE1_OUT"
TARGET_DTC_FLAGS="-q"
...
# ─────────────────────────────────────────────────────────────────────────────
# MAKE_ARGS — Why we do NOT pass LLVM=1:
#
# This kernel's top-level Makefile (lines 424-443) does:
#   ifneq ($(LLVM),)
#   CC    = clang      ← bare name, resolved from $PATH → system clang wins!
#   LD    = ld.lld
#   AR    = llvm-ar
#   ...
#
# Passing LLVM=1 causes those internal assignments to fire AFTER our
# command-line CC= is processed, clobbering it with the bare "clang" name.
# Fix: omit LLVM=1, pass every tool as an explicit absolute path instead.
# LLVM_IAS=1 is safe to keep — it only controls the integrated assembler.
#
# CROSS_COMPILE must be a bare prefix (not an absolute path).
# The Makefile does:  --target=$(notdir $(CROSS_COMPILE:%-=%))
# notdir strips the directory component, so an absolute path would give
# a correct triple. But to keep things clean we pass the bare prefix and
# rely on $PATH (set by setup_toolchains) to resolve gcc binaries.
# ─────────────────────────────────────────────────────────────────────────────
MAKE_ARGS=(
	"ARCH=$TARGET_ARCH"
	"SUBARCH=$TARGET_SUBARCH"
	# Absolute paths to Snapdragon Clang 19 — never resolved from $PATH
	"CC=$TARGET_CC"
	"CXX=$TARGET_CXX"
	"LD=$TARGET_LD"
	"AR=$TARGET_AR"
	...
	# Bare prefix — PATH (set by setup_toolchains) resolves the GCC binaries
	"CROSS_COMPILE=$TARGET_CROSS_COMPILE_PREFIX"
	"CROSS_COMPILE_COMPAT=$TARGET_CROSS_COMPILE_COMPAT_PREFIX"
	"CLANG_TRIPLE=$TARGET_CLANG_TRIPLE"
	# NO LLVM=1 — see comment above
	"LLVM_IAS=1"
...
check_tools(){
	...
	# Validate Snapdragon Clang binary directly (not system clang)
	[ -f "$TARGET_CC" ] || { echo -e "${RED}Snapdragon Clang not found at $TARGET_CC${NC}"; exit 1; }
	[ -f "$TARGET_LD" ] || { echo -e "${RED}Snapdragon LLD not found at $TARGET_LD${NC}"; exit 1; }
	...
	echo -e "${GREEN}Snapdragon Clang: $("$TARGET_CC" --version 2>&1 | head -1)${NC}"
...
# ---- Integrate ReSukiSU into kernel source ----
setup_resukisu() {
	echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
	echo -e "${CYAN}  Integrating ReSukiSU${NC}"
	echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

	# Must run from inside kernel source dir (setup.sh uses pwd as GKI_ROOT)
	cd "$KERNEL_DIR" || { echo -e "${RED}Cannot cd to KERNEL_DIR${NC}"; exit 1; }

	echo -e "${YELLOW}Running ReSukiSU setup script…${NC}"
	curl -LSs "https://raw.githubusercontent.com/ReSukiSU/ReSukiSU/main/kernel/setup.sh" | bash \
		|| { echo -e "${RED}✘ ReSukiSU setup failed!${NC}"; exit 1; }
	echo -e "${GREEN}✔ ReSukiSU integrated (KernelSU/ cloned, drivers/kernelsu symlink created)${NC}"

	# ── Verify / patch defconfig with all required ReSukiSU + SUSFS configs ──────
	local defconfig_file="$DEFCONFIG_PATH/$DEFCONFIG_NAME"
	if [ ! -f "$defconfig_file" ]; then
		echo -e "${RED}✘ Defconfig not found at $defconfig_file${NC}"
		exit 1
	fi

	echo -e "${YELLOW}Checking ReSukiSU configs in $defconfig_file…${NC}"

	# Helper: ensure a config=value line exists; add the whole block on first miss
	local block_added=0

	_ensure_config() {
		local key="$1"   # e.g. CONFIG_KSU
		local val="$2"   # e.g. y   (use "n" or a quoted string for others)

		if grep -q "^${key}=" "$defconfig_file"; then
			echo -e "${GREEN}  ✔ ${key} already set${NC}"
		else
			# Add header comment once before first missing entry
			if [ "$block_added" -eq 0 ]; then
				printf "\n# ReSukiSU\n" >> "$defconfig_file"
				block_added=1
			fi
			echo "${key}=${val}" >> "$defconfig_file"
			echo -e "${YELLOW}  + ${key}=${val} added${NC}"
		fi
	}

	# ── Core KSU ──────────────────────────────────────────────────────────
	_ensure_config "CONFIG_KSU"                                    "y"
	# CONFIG_KSU_DEBUG intentionally left out (not set by default)
	_ensure_config 'CONFIG_KSU_FULL_NAME_FORMAT'                   '"%TAG_NAME%-%COMMIT_SHA%@%REPO_NAME%"'
	...
	_ensure_config "CONFIG_KSU_SUSFS_SUS_MAP"                      "y"
