...
# AOSP GCC 4.9 prebuilts (used as linker stubs alongside Clang)
GCC64_URL="https://android.googlesource.com/platform/prebuilts/gcc/linux-x86/aarch64/aarch64-linux-android-4.9"
GCC32_URL="https://android.googlesource.com/platform/prebuilts/gcc/linux-x86/arm/arm-linux-androideabi-4.9"
GCC64_DIR="$(pwd)/../toolchains/aarch64-linux-android-4.9"
...
setup_toolchains() {
	...
	# ── AOSP GCC 4.9 AArch64 ────────────────────────────────────────────
	if [ -d "$GCC64_DIR/bin" ]; then
		echo -e "${GREEN}✔ GCC64 already present: $GCC64_DIR${NC}"
	else
		echo -e "${YELLOW}Cloning AOSP GCC 4.9 aarch64…${NC}"
		git clone --depth=1 "$GCC64_URL" "$GCC64_DIR" \
			|| { echo -e "${RED}✘ GCC64 clone failed!${NC}"; exit 1; }
		echo -e "${GREEN}✔ GCC64 ready${NC}"
	fi
	...
	if [ -d "$GCC32_DIR/bin" ]; then
		...
		echo -e "${YELLOW}Cloning AOSP GCC 4.9 arm…${NC}"
	...
	# ── Print compiler version for log ──────────────────────────────────
	COMPILER_STRING="$("$SDCLANG_DIR/bin/clang" --version 2>&1 | head -1)"
	CC_VERSION="$("$SDCLANG_DIR/bin/clang" --version 2>&1 | head -1)"
...
# Kick off toolchain setup immediately so PATH is ready for everything below
setup_toolchains
...
TARGET_CLANG_TRIPLE="aarch64-linux-android-"
TARGET_CROSS_COMPILE="aarch64-linux-android-"
TARGET_CROSS_COMPILE_COMPAT="arm-linux-androideabi-"
...
build_kernel() {
	...
	make $FINAL_KERNEL_BUILD_PARA 2>&1 | tee "$LOG_FILE"
