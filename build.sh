...
# Semi-automatic script suitable for use in Ubuntu, Debian
# ----------------------------------------------------------------------------
...
while true; do
	...
	case $choice in
		1)
			branch="Neutrino"
			git clone --depth=1 https://github.com/William24hmar/AnyKernel3.git -b master "$branch" "$AK3_PATH" &&
...
build_kernel() {
	...
	eval make $FINAL_KERNEL_BUILD_PARA 2>&1 | while IFS= read -r line; do

		CURRENT_SEC=$(date +%s)
		ELAPSED_SEC=$(( CURRENT_SEC - START_SEC ))

		TIME_FMT=$(printf "%02d:%02d" \
			$(( ELAPSED_SEC / 60 )) \
			$(( ELAPSED_SEC % 60 )))
	...
	grep -q '^CONFIG_MODULES=y$' "$TARGET_OUT/.config" && {
		eval make $FINAL_KERNEL_BUILD_PARA \
...
generate_flashable() {
	...
	grep -q '^CONFIG_MODULES=y$' "$TARGET_OUT/.config" 2>/dev/null && {
		...
		# Copy kernel modules
		find "$TARGET_OUT" -type f -name "*.ko" | while read -r module; do
