...
colors=($blue $yellow $white $red $green $magenta $lgreen $cyan $purple $pink $orange_yellow $greenish_yellow $blink_red $blink_green $blink_yellow)
...
echo -e "$blue << Toolchain download... >> $white"
	...
	if [ ! -d "gcc-arm" ] || [ ! -d "gcc-arm64" ]; then
		...
		for file in "${files[@]}"; do
			...
			if [ ! -f "$filename" ]; then
				...
				if aria2c -x 16 -s 16 -o "$filename" "$url" && [ $? -eq 0 ]; then
		...
		echo "Directories 'gcc-arm' and 'gcc-arm64' already exist."
	fi
	...
	if [[ ! -d "$CLANG_VERSION" ]]; then
		...
		mv bin build.info lib share clang/
	else
...
if [[ ! -d "$ANYKERNEL3_VERSION" ]]; then
	...
	# Clone only Phone1 version
	git clone --depth=1 https://github.com/William24hmar/AnyKernel3.git -b master "${ANYKERNEL3_VERSION}"
else
...
obj_builder()
{
	...
	if [[ $TEST == "1" ]]; then
		rm -rf arch/arm64/configs/phone1-${SUFFIX}_defconfig
	fi
	if [[ $DTB_ZIP != "1" ]]; then
		exit 0
	fi
...
zipper()
{
	...
	# Info ZIP
	echo -e "The kernel has successfully been compiled and can be found in $(pwd)/${greenish_yellow}${ANYK_VERSION}-${BUILD_TIME}-signed.zip $white"
