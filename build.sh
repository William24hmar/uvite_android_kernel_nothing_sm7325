...
echo -e "${CYAN}2.👉 NDK CLANG / RANDOM (Classic)${NC}"
...
[ "$COMPILER_CHOICE" == "1" ] && {
	...
	echo -e "${CYAN}Staying with Clang. Classic choice.${NC}"
...
display_build_info(){
	echo -e "${PURPLE}***************Spaceware-Kernel**************${NC}"
...
clone_anykernel3(){
	while true; do
		...
		echo -e "${BLUE}1.👉 Spaceware-main (William24hmar/AnyKernel3)${NC}"
		echo -e "${BLUE}2.👉 Custom git clone command${NC}"

		# Set timeout for user input (5 seconds)
		read -t 5 -rp "Enter your choice (1 or 2): " choice

		# If no input is provided within 5 seconds, default to action 1 (Spaceware-main)
		[ -z "$choice" ] && echo -e "${YELLOW}No input detected. Automatically selecting Spaceware-main.${NC}" && choice=1
...
generate_flashable() {
	...
	FLASHABLE_ZIP="Spaceware-$CLEAN_TIME.zip"
	SIGNED_ZIP="Spaceware-$CLEAN_TIME-signed.zip"
...
display_kernel_version_info() {
	...
	SPACEWARE_DEFCONFIG=$(find . -path '*/vendor/lahaina-qgki_defconfig' -print -quit)

	[ -z "$SPACEWARE_DEFCONFIG" ] && echo -e "${RED}vendor/lahaina-qgki_defconfig not found!${NC}" && return 1
	...
	echo -e "CONFIG_LOCALVERSION: $(grep -E '^CONFIG_LOCALVERSION=' $SPACEWARE_DEFCONFIG | awk -F'=' '{print $2}')"
	echo -e "CONFIG_UNAME_OVERRIDE_STRING: $(grep -E '^CONFIG_UNAME_OVERRIDE_STRING=' $SPACEWARE_DEFCONFIG | awk -F'=' '{print $2}')"
	echo -e "${PURPLE}**********************************************${NC}"
}
