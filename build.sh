...
build_make_args() {
    MAKE_ARGS=(
        ...
        # ── 32-bit compat: EVA GCC arm-eabi (compile.sh line 499) ────────────
        "CC_COMPAT=$GCC32_DIR/bin/arm-eabi-gcc"
        "CROSS_COMPILE_COMPAT=arm-eabi-"
        # ── GCC optimisation flags (compile.sh line 499) ──────────────────────
        "GCC_LTO=1"
        "GRAPHITE=1"
        # ── PATH override so make itself resolves aarch64-elf-* from EVA GCC ──
        ...
        "HOSTCXX=g++"
        "HOSTLD=ld"
        # ── HOSTCFLAGS: force C11 for host tools ──────────────────────────────
        # Ubuntu GCC 14 defaults to -std=gnu23 where bool/false/true are
        # reserved keywords. Linux 5.4 kernel headers use them as typedef/enum
        # values (legal in C11/C17). This causes:
        #   stddef.h:11: error: cannot use keyword 'false' as enumeration constant
        #   types.h:30:  error: 'bool' cannot be defined via 'typedef'
        # Forcing -std=gnu11 restores the pre-C23 behaviour for host builds.
        ...
        # nfc_i2c_drv.c — size_t with %d, implicit fallthrough
