...
static inline void util_est_update(struct sched_entity *se)
{
	 ...
	 */
	if ((ue.enqueued + UTIL_EST_MARGIN) < READ_ONCE(se->avg.runnable_avg))
		goto done;
...
static bool throttle_cfs_rq(struct cfs_rq *cfs_rq)
{
	...
	long task_delta, runnable_delta, idle_delta, dequeue = 1;
...
void unthrottle_cfs_rq(struct cfs_rq *cfs_rq)
{
	...
	struct cfs_bandwidth *cfs_b = tg_cfs_bandwidth(cfs_rq->tg);
	struct sched_entity *se;
	long task_delta, runnable_delta, idle_delta;
...
idle:
	if (rf) {
		...
		/*
		 * Because sched_balance_newidle() releases (and re-acquires) rq->lock,
		 * it is possible for any higher priority task to appear. In that
		 * case we must re-start the pick_next_entity() loop.
		 */
		if (new_tasks < 0)
			return RETRY_TASK;
